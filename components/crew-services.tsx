...
function EmptyPage({ page, onAction }: { page: string; onAction: () => void }) {
  ...
  return <div className="page-body"><div className="welcome-row"><div><p className="date-line">{copy.kicker}</p><h2>{copy.title}</h2><p className="muted">{copy.description}</p></div><button className="primary-btn" onClick={onAction}><Plus size={16} /> {copy.action}</button></div><section className="empty-dashboard page-empty"><div className="empty-icon"><Icon size={24} /></div><h3>{page === 'Notifications' ? 'No new notifications' : `${copy.title} is ready`}</h3><p>{copy.description} Start by adding your first record to get started.</p><button className="text-btn" onClick={onAction}>{copy.action}</button></section></div>
}
